        ...
        if (sequenceSectionPilares) {
            ...
            if (rect.top <= 0 && rect.bottom >= windowHeight) {
                ...
                if (isMobile) {
                    ...
                    // ESCRITORIO: Coreografía Exacta 
                    pilaresDesktopCards.forEach((card, index) => {
                        // Empiezan a aparecer tras el 15% de scroll para que se vea limpio el título antes
                        const start = 0.15 + (index * 0.2); 
                        let p = (progress - start) / 0.2;
                        if (p < 0) p = 0; if (p > 1) p = 1;

                        // Se ocultan al 100% si no ha llegado su turno
                        card.style.opacity = p > 0 ? 1 : 0; 

                        // RUTAS DE ENTRADA CORREGIDAS
                        let x = 0, y = 0;
                        if(index === 0) { x = -100; y = -100; } // SICE (Top Left) -> Entra desde superior izquierda
                        else if(index === 1) { x = 100; y = -100; } // Vista Alegre (Top Right) -> Entra desde superior derecha
                        else if(index === 2) { x = -100; y = 100; } // NUCTECH (Bottom Left) -> Entra desde inferior izquierda
                        else if(index === 3) { x = 100; y = 100; }  // ENTELAR (Bottom Right) -> Entra desde inferior derecha
                        
                        card.style.transform = `translate(${x * (1 - p)}%, ${y * (1 - p)}%)`;
                        card.style.borderRadius = `${80 * (1 - p)}px`; // De curvas a rectas al encajar
                    });
                }
            }
        }

        // 3. ANIMACIÓN ECOSISTEMA (Como el Dilema)
